...
[![build and tests](https://github.com/Slash-ui/slash-ascii/actions/workflows/ci.yml/badge.svg?branch=main)](https://github.com/Slash-ui/slash-ascii/actions/workflows/ci.yml)
[![security](https://github.com/Slash-ui/slash-ascii/actions/workflows/security.yml/badge.svg?branch=main)](https://github.com/Slash-ui/slash-ascii/actions/workflows/security.yml)
[![docs](https://github.com/Slash-ui/slash-ascii/actions/workflows/pages.yml/badge.svg?branch=main)](https://slash-ui.github.io/slash-ascii/)

[![npm](https://img.shields.io/npm/v/slash-ascii?color=5F53EF&label=npm)](https://www.npmjs.com/package/slash-ascii)
[![downloads](https://img.shields.io/npm/dm/slash-ascii?color=5F53EF)](https://www.npmjs.com/package/slash-ascii)
...
**[Read the guide →](https://slash-ui.github.io/slash-ascii/)**
...
`npm run build` comes before `npm test` because the command line suite drives
`dist/cli.js` when it exists — the artefact users actually run, and plain
JavaScript that loads the same way on every platform. Without a build it falls
back to the TypeScript source through tsx.

### What runs on a pull request

| Workflow | What it checks |
| --- | --- |
| `ci` | Typecheck, build, tests and the site build, on Linux, macOS and Windows against Node 20 and 22. Also that every commit message and the pull request title parse as conventional commits |
| `security` | gitleaks over the full history, `npm audit`, and CodeQL. Also runs weekly, which is what catches an advisory published after the last commit |
| `pages` | Rebuilds <https://slash-ui.github.io/slash-ascii/> from `site/` on every push to `main` |

The audit gate blocks on a **high** advisory anywhere in the runtime tree, and
on a **critical** one anywhere at all. Runtime is the stricter of the two on
purpose: `sharp` decodes files that came from somewhere else.

The site is built by `npm run build:site`, which renders its own examples by
calling the library. If rendering changes, the page changes with it — it cannot
document behaviour the code no longer has.
